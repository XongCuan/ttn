...
namespace TCore\Support\Repositories\Eloquent;
...
abstract class RepositoryAbstract implements RepositoryInterface
{
    ...
    public function getByQueryBuilder(array $filter, array $relations = [], $sort = ['id', 'desc'])
    {
        ...
        if(empty($sort) == false)
        {
            $this->instance = $this->instance->orderBy(...$sort);
        }
        ...
        foreach($filter as $field => $value)
        {
            if (is_array($value))
            {
                ...
                $this->instance = match (strtoupper($condition)) {
                    ...
                    'DATE' => $this->instance->whereDate($field, $val),
                    'MONTH' => $this->instance->whereMonth($field, $val),
                    'YEAR' => $this->instance->whereYear($field, $val),
                    'NOT_IN' => $this->instance->whereNotIn($field, $val),
                    'BETWEEN' => $this->instance->whereBetween($field, $val),
                    'RELATION' => $this->instance->whereRelation($field, ...$val),
                    'HAS' => $this->instance->whereHas($field, ...$val),
                    'HAS_ONE' => $this->instance->has($field),
                    'DOESNTHAVE' => $this->instance->doesntHave($field),
