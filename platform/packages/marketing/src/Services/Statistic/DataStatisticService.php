...
namespace TCore\Marketing\Services\Statistic;
...
class DataStatisticService
{
    ...
    public function revenueFollowDate()
    {
        ...
        {
            $filter[] = ['order', 'relation', ['customer_type', $this->customerType]];
        }

        if($this->source_id)
        {
            $filter[] = ['order', 'has', fn($q) => $q->whereRelation('customer', 'source_id', $this->source_id)];
        }

        $orderPayment = OrderPayment::makeQuery(filter: $filter, sort: ['revenue_date', 'asc'])
        ->selectRaw('DATE_FORMAT(created_at, "%d-%m-%Y") as revenue_date, SUM(amount) as revenue')
        ->groupByRaw('DATE_FORMAT(created_at, "%d-%m-%Y")');

        $revenues = $orderPayment
    ...
    public function salesFollowDate()
    {
    ...
    public function totalRenueve(bool $samePeriod = false)
    {
        ...
        $totalOrder = OrderPayment::makeQuery($filter)->sum('amount');

        return $totalOrder;
    }

    public function totalSales(bool $samePeriod = false)
    {
