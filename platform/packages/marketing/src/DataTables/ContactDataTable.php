...
class ContactDataTable extends DataTables
{
    ...
    {
        $this->columnHasSearch = ['info', 'status', 'admin_id', 'date_add'];
    }

    protected function setColumnSearchDate(): void
    {
        $this->columnSearchDate = ['date_add'];
    }
    ...
    {
        if(get_auth_admin()->hasLeaderShipRole(Department::Marketing) == false)
        {
            $this->removeColumns = [
                'assigns', 'admin_id'
            ];
    ...
    {
        return $this->model->makeQuery(relations: ['source', 'type', 'sector', 'rangePrice', 'province', 'district', 'ward', 'assigns', 'creator'])->followByMkt();
    }
    ...
    protected function setEditColumns(): void
    {
        $this->editColumns = [
            ...
            'admin_id' => fn($data) => view('packages_marketing::contacts.datatables.columns.creator')->with('data', $data),
            'date_add' => fn($row) => format_date($row->date_add)
        ];
    }
    ...
    {
        $this->addColumns = [
            'info' => fn($data) => view('packages_marketing::contacts.datatables.columns.info')->with('data', $data),
            'taxonomy' => fn($data) => view('packages_marketing::contacts.datatables.columns.taxonomy')->with('data', $data),
            'action' => 'packages_marketing::contacts.datatables.columns.action',
