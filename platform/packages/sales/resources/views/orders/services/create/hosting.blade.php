...
@if (isset($required) == false ||  (isset($required) && $required))
    <x-core_base::input type="hidden" name="is_service_hosting" value="1" />
@endif
<div class="row">
    <div class="col-12 mb-3">
        <label for="" class="form-label">@lang('Số tiền'):</label>
        <x-core_base::input type="text" class="inp-number-format" name="service[{{$type}}][amount]" :required="$required ?? true" :placeholder="trans('Số tiền')" />
    </div>
