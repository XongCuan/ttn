    ...
    <div class="card mt-3">
        ...
        <div class="card-body p-2">
            <x-core_base::select class="select2-bs5-ajax-many select2-condition"
                data-condition="select[name='assigns[]']" name="team_id" :data-url="route('internal.select_team')" :required="true">
                <x-core_base::select.option :selected="$project->team_id" :value="$project->team_id" :title="$project->teamInternal->name" />
            </x-core_base::select>
        </div>
    </div>
    <div class="card mt-3">
